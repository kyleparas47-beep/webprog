...
<head>
    ...
    <link rel="stylesheet" href="../../assets/css/calendar_admin.css">
...
<body>
    ...
    <script>
        ...
        function onScanSuccess(decodedText, decodedResult) {
            ...
            // Always verify the ticket regardless of pause success
        ...
        function verifyTicket(ticketNumber) {
            ...
            fetch('../../api/attendance/verify_ticket.php', {
                method: 'POST',
            ...
            .then(data => {
                ...
                displayResult(data);
                // Reset processing flag (resumeScanner is handled by displayResult or confirmAttendance)
                isProcessing = false;
            })
            .catch(error => {
                console.error('Verification error:', error);
                showError('Verification failed. Please try again.');
                // Reset processing flag and resume scanning after network error
                isProcessing = false;
                setTimeout(() => {
                    resumeScanner();
                }, 2000);
        ...
        function resumeScanner() {
            // Resume the QR scanner if it exists and is paused
            if (html5QrCode) {
                try {
                    const state = html5QrCode.getState();
                    console.log('Scanner state:', state);
                    
                    // State 2 = PAUSED, State 3 = SCANNING
                    if (state === 2) {
                        html5QrCode.resume()
                            .then(() => {
                            ...
                            .catch(err => {
                                ...
                                document.getElementById('scanner-status').innerHTML = '<i class="fas fa-exclamation-triangle"></i> Scanner error - Please refresh page';
                            });
                    } else if (state === 3) {
                        // Already scanning
                        document.getElementById('scanner-status').innerHTML = '<i class="fas fa-camera"></i> Ready for next scan';
                    } else {
                        // Scanner stopped, need to restart
                        console.log('Scanner stopped, reinitializing...');
                        initQRScanner();
                    }
                } catch (e) {
        ...
        function displayResult(data) {
            ...
            if (data.success) {
                if (data.already_attended) {
                    ...
                    // Auto-hide warning and resume scanner after 3 seconds
                    setTimeout(() => {
                ...
                // Auto-hide error and resume scanner after 3 seconds
                setTimeout(() => {
                    resultCard.style.display = 'none';
                    resumeScanner();
                }, 3000);
        ...
        function confirmAttendance() {
            ...
            fetch('../../api/attendance/mark_attendance.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Resume scanner immediately for next student
                    resumeScanner();
                } else {
